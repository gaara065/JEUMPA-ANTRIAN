...
<audio id="c" src="suara/c.wav"  ></audio>
...
<?php foreach($loket_prioritas as $value){ ?>
<audio id="suarabelloket<?php echo $value; ?>" src="suara/<?php echo $value; ?>.wav"  ></audio>
<?php } ?>
<?php
 $get_antrian = fetch_assoc(query("SELECT antrian FROM antriprioritas"));
 $tcounter = $get_antrian['antrian'];
 $_tcounter = $tcounter + 1;
 if(isset($_GET['loket'])) {
	 query("UPDATE antrian_loket SET end_time = CURRENT_TIME() WHERE type = 'Prioritas' AND noantrian = '{$tcounter}'");
	 query("UPDATE antriprioritas SET loket = '{$_GET['loket']}', antrian = '{$_tcounter}'");
 }
...
if($_SERVER['REQUEST_METHOD'] == "POST") {
  if(empty($_POST['antrian'])) {
    query("DELETE FROM `antriprioritas`");
    query("INSERT INTO `antriprioritas` (`loket`, `antrian`) VALUES ('0', '0')");
  } else {
    query("UPDATE antriprioritas SET loket = '{$_GET['loket']}', antrian = '{$_POST['antrian']}'");
  }
}
?>
<div align="center" style="font-size: 64px;color:white; text-shadow: 2px 2px 4px #000000;" class="mt-2 mb-5">Pemanggil Antrian Prioritas</div>
			...
			<?php foreach($loket_prioritas as $value){ ?>
				<div class="col-xs-12 col-sm-6 col-md-4 col-lg-4 mb-5">
			      ...
			      <div class="card-header" style="font-size:32px;">Loket <?php echo $value; ?></div>
				    <div class="card-body">
							<h5 class="card-title" style="font-size:72px;">C<?php echo $antrian; ?></h5>
				    </div>
				    <div class="card-footer p-0">
							<div class="btn-group btn-group-justified">
							  <a href="#" class="btn btn-primary" style="font-size:32px;"><?php $q = fetch_assoc(query("SELECT MAX(noantrian) as q FROM antrian_loket WHERE type = 'Prioritas' AND postdate LIKE current_date()")); if(empty($q['q'])) { echo '0'; } else { echo $q['q']; } ?></a>
							  <a href="#" class="btn btn-primary" style="font-size:32px;"><i class="fas fa-bullhorn" onclick="mulai<?php echo $value; ?>();"></i></a>
							  <a href="antrian.php?action=panggil_prioritas&loket=<?php echo $value; ?>" class="btn btn-primary" style="font-size:32px;"><i class="fas fa-forward"></i></a>
							</div>
				    </div>
				  </div>
					</div>
...
function mulai<?php echo $value; ?>(){
	...
	setTimeout(function() { $("#c")[0].play() }, totalwaktu);
