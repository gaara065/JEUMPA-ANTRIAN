<?php
    	...
    	if(isset($_POST['next_current']) and ($_POST['next_current'] != NULL)){
			...
			$rstClient = $mysqli->query('SELECT urutan as count FROM data_antrian WHERE status != 4 and gru=1 order by id desc');
			...
			if($rowClient['count']>0){
				...
				$results = $mysqli->query('INSERT INTO data_antrian (waktu,status,gru,urutan) VALUES ("'.date("Y-m-d H:i:s").'",3,1,"'.$jmlClient.'")');
				$id = $mysqli->insert_id;
			    echo json_encode( array('next'=> $id) );
