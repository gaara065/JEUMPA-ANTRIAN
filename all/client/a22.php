...
{
 	$idA = $_POST['idnyaa']; //id
	$results = $mysqli->query('UPDATE data_antrian SET counter=3 WHERE id='.$idA.'');
	?>
<script>
window.location = 'a22.php';
</script>
...
$results = $mysqli->query('DELETE FROM data_antrian whereid='.$idA.'');
...
<?php
	...
	$results = $mysqli->query("SELECT * FROM data_antrian where (status='3' or status='2' or status='0') and gru='1' and (counter='2') order by id asc");	
		...
		$idaa = $aa['id'];
			...
			$results = $mysqli->query("SELECT * FROM data_antrian where (status='3' or status='2' or status='0') and gru='2' and (counter='2') order by id asc");	
		$aa = $results->fetch_array();
		$idb = $aa['urutan'];
		$idab = $aa['id'];
		
		
			$results = $mysqli->query("SELECT * FROM data_antrian where (status='3' or status='2' or status='0') and gru='3' and (counter='2') order by id asc");	
		$aa = $results->fetch_array();
		$idc = $aa['urutan'];
		$idac = $aa['id'];
...
else if ($idc=='')
{
	...
	$resultsa = $mysqli->query("SELECT count(*) as sisa FROM data_antrian where (status='3' or status='2' or status='0') and gru='1' and (counter='2') ");	
		$ra = $resultsa->fetch_array();	
		
	$resultsb = $mysqli->query("SELECT count(*) as sisa FROM data_antrian where (status='3' or status='2' or status='0') and gru='2' and (counter='2') ");	
		$rb = $resultsb->fetch_array();

$resultsc = $mysqli->query("SELECT count(*) as sisa FROM data_antrian where (status='3' or status='2' or status='0') and gru='3' and (counter='2') ");	
 ...
 <font style="font-size:40px"><b><?php  echo "$ida"; ?></font>
  ...
  <script type="text/javascript" src="jquery.maskMoney.min.js"></script>
   ...
   <button type="submit" id="proceed" class="btn btn-primary" name="submita1">Suara</button>
     ...
     <button type="submit" id="proceeda4" class="btn btn-warning" name="submita3">Lewati</button>
<br/>
<br/>
