 ...
 if (isset($_POST['submita2']))
{
	...
	$results = $mysqli->query('UPDATE data_antrian SET counter=5 WHERE id='.$idA.'');
	?>
<script>
window.location = 'a42.php';
</script>
...
<?php
	...
	$results = $mysqli->query("SELECT * FROM data_antrian where (status='3' or status='2' or status='0') and gru='1' and (counter='4') order by id asc");	
		...
		$idaa = $aa['id'];
			...
			$results = $mysqli->query("SELECT * FROM data_antrian where (status='3' or status='2' or status='0') and gru='2' and (counter='4') order by id asc");	
		$aa = $results->fetch_array();
		$idb = $aa['urutan'];
		$idab = $aa['id'];
		
		
			$results = $mysqli->query("SELECT * FROM data_antrian where (status='3' or status='2' or status='0') and gru='3' and (counter='4') order by id asc");	
		$aa = $results->fetch_array();
		$idc = $aa['urutan'];
		$idac = $aa['id'];
...
else if ($idc=='')
{
	...
	$resultsa = $mysqli->query("SELECT count(*) as sisa FROM data_antrian where (status='3' or status='2' or status='0') and gru='1' and (counter='4') ");	
		$ra = $resultsa->fetch_array();	
		
	$resultsb = $mysqli->query("SELECT count(*) as sisa FROM data_antrian where (status='3' or status='2' or status='0') and gru='2' and (counter='4') ");	
		$rb = $resultsb->fetch_array();

$resultsc = $mysqli->query("SELECT count(*) as sisa FROM data_antrian where (status='3' or status='2' or status='0') and gru='3' and (counter='4') ");	
